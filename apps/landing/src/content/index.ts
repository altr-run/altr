...
export const COMPARE_PAGES: Record<string, ComparePage> = {
	"altr-vs-cursor": {
		...
		featureTable: [
			...
			{ feature: "Spec editor with agent assist", altr: true, them: false, altrNote: "Pax writes specs from thread signal", themNote: null },
	...
	"altr-vs-devin": {
		...
		featureTable: [
			...
			{ feature: "Autonomous code execution", altr: true, them: true, altrNote: "Eng agent in autopilot mode", themNote: "Devin's core feature" },
...
export const USE_CASES: Record<string, UseCasePage> = {
	"feature-delivery": {
		...
		steps: [
			...
			{ step: 2, label: "Pax writes the spec", body: "Pax turns the thread into a structured spec: headline, problem statement, acceptance criteria, open questions. You review and edit. Nothing gets resolved without a human in the loop." },
			{ step: 3, label: "Eng opens the PR", body: "Eng works the spec in an isolated git worktree, streams progress back to you, and opens a PR with the spec and acceptance criteria attached. Reviewers see the why alongside the diff." },
	...
	"bug-triage": {
		...
		steps: [
			...
			{ step: 2, label: "Structure the issue", body: "Pax turns the raw signal into a structured issue: observed vs expected behavior, severity, affected surface, reproduction steps. No information lost." },
	...
	"pr-review": {
		...
		steps: [
			{ step: 1, label: "Spec attached at open", body: "Altr attaches the spec -- acceptance criteria, open questions, original thread -- to every PR Eng opens. The context is there from the first comment." },
	...
	"incident-follow-up": {
		...
		steps: [
			...
			{ step: 2, label: "Scope the fix", body: "Pax extracts the agreed remediation from the postmortem and structures it as a spec: what broke, what was decided, what the fix should do, what it should not change." },
			{ step: 3, label: "Implement with context", body: "Eng works the fix with the full incident context attached. The PR references the postmortem and the incident thread -- so reviewers can verify the fix matches what was decided." },
		],
	...
	"migrations": {
		...
		steps: [
			{ step: 1, label: "Capture the migration brief", body: "Altr reads the design doc, the meeting notes, or the Slack thread where the migration was decided. Pax structures a migration spec: scope, constraints, rollback plan, acceptance criteria." },
			{ step: 2, label: "Break into supervised tasks", body: "Pax proposes a task breakdown. Each task runs in its own worktree. You control the autonomy level per task -- some automated, some with review gates." },
	...
	"release-follow-through": {
		...
		steps: [
			...
			{ step: 2, label: "Generate release notes", body: "Pax reads the merged PRs and their attached specs to generate accurate, audience-appropriate release notes. Human edits before publish." },
...
export const INTEGRATIONS: Record<string, IntegrationPage> = {
	...
	github: {
		...
		howItWorks: "When Eng opens a PR, Altr attaches the spec and the original execution trail -- Slack thread, acceptance criteria, open questions. Reviewers see the why alongside the diff. Altr also reads existing GitHub issues and can route them into the Altr execution trail alongside Slack signals.",
		signals: [
			{ signal: "PR open events", description: "Altr attaches spec context when Eng opens a PR." },
	...
	linear: {
		...
		howItWorks: "Altr creates Linear tickets from Pax-generated specs and keeps status in sync as Eng works. Linear remains your source of truth for project tracking. Altr adds the layer Linear cannot provide: the original Slack thread, the full spec, and the acceptance criteria -- attached to every ticket so they travel with the work.",
		signals: [
			{ signal: "Ticket creation from spec", description: "Pax creates structured Linear tickets from approved specs." },
			{ signal: "Status sync", description: "Ticket status updates as Eng progresses -- in-progress, in-review, done." },
	...
	notion: {
		...
		howItWorks: "Product specs, design briefs, and architecture decisions often live in Notion. Altr reads designated Notion pages and attaches their content to the execution trail -- so when Pax writes a spec, it has access to the full background doc, not just the summary someone typed into a Slack thread.",
		signals: [
			{ signal: "Product specs", description: "Full product requirement docs attached to Pax specs as source context." },
...
export const CHANGELOG: Record<string, ChangelogEntry> = {
	"v0-1-2": {
		...
		type: "new",
		summary: "Thread capture, spec editor, and Pax agent available to pilot teams. Mac-native via Tauri 2. Local-first SQLite. BYOK for Anthropic.",
		body: null,
		metadata: null,
	},
	"v0-1-1": {
	...
	"v0-1-0": {
		...
		type: "new",
		summary: "Altr is live for the first pilot cohort. Thread capture, Pax spec generation, and the execution trail graph are live. Eng worktree support in the next release.",
		body: null,
		metadata: null,
	},
	"v0-0-9": {
		...
		type: "fixed",
		summary: "Fixed streaming drops on long Pax sessions in the spec editor. Token budget tracking now matches the actual stream state rather than estimating from message count.",
		body: null,
		metadata: null,
	},
}
