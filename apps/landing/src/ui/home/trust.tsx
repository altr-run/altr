...
export default function Trust() {
	return (
		<section
			id="security"
			className="py-[140px] px-8 bg-zinc-950 text-white"
		>
			<div className="inner flex flex-col gap-14">
				<Reveal className="flex flex-col items-start gap-5 max-w-[640px]">
					...
					<h2
						className="font-serif font-normal text-white leading-[1.06] tracking-[-0.03em] m-0"
					...
					<p
						className="font-sans text-white/60 leading-[1.65] m-0 max-w-[46ch]"
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-[1px] bg-white/8 rounded-[var(--r-lg)] overflow-hidden border border-white/10">
					{PILLARS.map((p, i) => (
						<Reveal
							key={p.title}
							delay={i * 90}
							className="bg-white/[3%] p-9 flex flex-col gap-5 group transition-[background] duration-300 hover:bg-white/[6%]"
						>
							...
							<h3 className="font-serif font-normal text-[20px] text-white m-0 leading-[1.25] tracking-[-0.01em]">
								{p.title}
							</h3>
							<p className="font-sans text-[14px] text-white/55 leading-[1.72] m-0">
								{p.body}
							</p>
						</Reveal>
					))}
				...
				<Reveal>
					<div className="flex items-center gap-3 font-mono text-[10.5px] tracking-[0.06em] text-white/35 flex-wrap">
						{[
							...
							...(i > 0
								? [<span key={`dot-${i}`} className="w-[3px] h-[3px] rounded-full bg-white/20 shrink-0" />]
								: []),
				...
				<Reveal className="flex items-center justify-between border-t border-white/10 pt-7 flex-wrap gap-4">
					<div className="flex items-center gap-2.5 flex-wrap font-mono text-[10.5px] text-white/30 tracking-[0.06em]">
						<span className="uppercase">Works with your existing stack:</span>
						{['Slack', 'GitHub', 'Linear', 'Notion', 'Figma', 'PagerDuty'].map((t) => (
							<span
								key={t}
								className="border border-white/15 rounded-full px-2 py-0.5 text-white/40"
							>
								{t}
							</span>
						))}
